...
func persistentalloc(size, align uintptr, sysStat *sysMemStat) unsafe.Pointer {
	var p *notInHeap
	p = (*notInHeap)(persistentalloc1(size, align, sysStat))
...
func persistentalloc1(size, align uintptr, sysStat *sysMemStat) *notInHeap {
	...
	if size >= maxBlock {
		return (*notInHeap)(sysAlloc(size))
	}
	...
	if persistent.off+size > persistentChunkSize || persistent.base == nil {
		persistent.base = (*notInHeap)(sysAlloc(persistentChunkSize))
		...
		for {
			...
			*(*uintptr)(unsafe.Pointer(persistent.base)) = chunks
			// fmt.Println(persistentChunks == nil)
			if atomic.CompareAndSwapUintptr((*uintptr)(unsafe.Pointer(&persistentChunks)), chunks, uintptr(unsafe.Pointer(persistent.base))) {
				break
			}
			// if atomic.CompareAndSwapPointer(&persistentChunks), chunks, uintptr(unsafe.Pointer(persistent.base))) {
			// 	break
			// }
		}
		persistent.off = alignUp(PtrSize, align)
	}
...
func (a *addrRanges) cloneInto(b *addrRanges) {
	if len(a.ranges) > cap(b.ranges) {
		...
		ranges.array = (*notInHeap)(persistentalloc(unsafe.Sizeof(addrRange{})*uintptr(ranges.cap), PtrSize, b.sysStat))
	}
